<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=00ADB5&height=200&section=header&text=Tima%20Dawwa&fontSize=70&fontColor=ffffff&desc=AI%20Specialist%20%7C%20Data%20Analyst%20%7C%20Flutter%20Developer&descAlignY=70&descSize=20&animation=twinkling" />
</div>

<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&duration=4000&pause=1000&color=00ADB5&center=true&vCenter=true&width=800&height=60&lines=AI+Engineer;IT+Graduate+(Ranked+Highest:+98%25);Data+Analyst+%26+Problem+Solver;Building+Intelligent+%26+Scalable+Solutions;Creative+Technologist" alt="Typing SVG" />
</div>

<p align="center">
  <strong>Creative Technologist | Problem Solver</strong><br>
  <em>Bridging the gap between cutting-edge AI and real-world applications.</em>
</p>

<p align="center">
  <a href="mailto:tima302t@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://sy.linkedin.com/in/tima-dawwa-698b13267"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://github.com/Tima-Dawwa"><img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
</p>

<div align="center">
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.png" width="800">
</div>

## 👨‍💻 About Me

💡 I am a newly graduated **AI Specialist** and **Flutter Developer** with a passion for building innovative systems. I recently graduated with a **BSc in Information Technology (AI Specialization)** from Damascus University, achieving the **highest mark of 98%** on my graduation project! 🏆

My work spans across deep learning models, natural language processing, data analysis, and cross-platform mobile applications, always with a focus on creating impactful and scalable outcomes.

- 🎓 **Education:** BSc in Information Technology, AI Specialization – *Damascus University* (Graduated with 98% in Grad Project)
- 🔭 **Currently working on:** Advanced AI models, NLP pipelines, and real-time mobile applications.
- 🌱 **Currently learning:** Advanced LLM architectures, RAG systems, and scalable deployment.
- 💬 **Ask me about:** Python, Machine Learning, Flutter, React, and Data Analysis.
- 🌍 **Languages:** Arabic 🇸🇾 | English 🌍

<div align="center">
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/aqua.png" width="800">
</div>

## 🛠️ Tech Stack & Tools

*A stunning visual representation of the technologies I work with:*

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=py,cpp,cs,java,dart,js,tensorflow,pytorch,opencv,flutter,react,html,css,git,firebase,github&perline=8&theme=dark" alt="Skill Icons" />
  </a>
</p>

<div align="center">
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/colored.png" width="800">
</div>

## 🚀 Featured Projects

| Project | Description | Tech Stack |
|---------|-------------|------------|
| [**Dermatology Smart Expert System**](https://github.com/Tima-Dawwa/Dermatology-Smart-Expert-System) | An intelligent system for dermatological diagnosis. | Python, ML |
| [**RecoMindSystem**](https://github.com/Tima-Dawwa/RecoMindSystem) | A personalized recommendation engine built with advanced algorithms. | Jupyter, Python |
| [**NLP with Disaster Tweets**](https://github.com/Tima-Dawwa/Natural-Language-Processing-with-Disaster-Tweets) | Natural Language Processing model to classify emergency tweets. | NLP, Scikit-Learn |
| [**Subjectivity in News Articles**](https://github.com/Tima-Dawwa/Subjectivity-in-News-Articles) | Analyzing bias and subjectivity in news media using Machine Learning. | Jupyter, NLP |
| [**OCR Signature**](https://github.com/Tima-Dawwa/ocr_signature) | Optical Character Recognition system for signature extraction. | JavaScript |
| [**Drug Distribution MPI**](https://github.com/Tima-Dawwa/Drug-Distribution-Mpi) | A scalable drug distribution system built using Message Passing Interface. | C# |

<div align="center">
  <img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.png" width="800">
</div>

## 📊 GitHub Analytics

<p align="center">
  <a href="https://github.com/Tima-Dawwa">
    <img src="https://github-readme-stats.vercel.app/api?username=Tima-Dawwa&show_icons=true&theme=synthwave&hide_border=true" alt="Tima's GitHub Stats" />
  </a>
  <a href="https://github.com/Tima-Dawwa">
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=Tima-Dawwa&theme=synthwave&hide_border=true" alt="Tima's GitHub Streak" />
  </a>
</p>

<p align="center">
  <a href="https://github.com/Tima-Dawwa">
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Tima-Dawwa&layout=compact&theme=synthwave&hide_border=true" alt="Top Languages" />
  </a>
</p>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=Tima-Dawwa&label=Profile%20views&color=00ADB5&style=flat" alt="Profile views" />
</div>
